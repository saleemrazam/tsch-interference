...
PROCESS_THREAD(node_process, ev, data)
{
  ...
  int is_coordinator;
  if (node_id == 2) {
    printf("SPD: dest is n1\n");
    linkaddr_copy(&dest,&n1);
  } else if (node_id == 3) {
    printf("SPD: dest is n4\n");
    linkaddr_copy(&dest,&n4);
  } else if (node_id == 5) {
    printf("SPD: dest is n6\n");
    linkaddr_copy(&dest,&n6);
  } else if (node_id == 7) {
    printf("SPD: dest is n8\n");
    linkaddr_copy(&dest,&n8);
  } else {
    printf("SPD: no dest\n");
    linkaddr_copy(&dest,&linkaddr_null);
  }

  is_coordinator = (linkaddr_cmp(&linkaddr_node_addr,&n1) ||
                    linkaddr_cmp(&linkaddr_node_addr,&n4) ||
                    linkaddr_cmp(&linkaddr_node_addr,&n6) ||
                    linkaddr_cmp(&linkaddr_node_addr,&n8));
  ...
  while(1) {
    if(node_id != 1 && node_id != 4 && node_id != 6 && node_id != 8 && tsch_is_associated){
      nt_send_packet();
    }
